...
class LoginForm(forms.Form):
    username_or_email = forms.CharField(
        label='用户名或邮箱', required=True, widget=forms.TextInput(attrs={'class': 'form-control', 'placeholder': '请输入用户名或邮箱'}))
...
class ChangeNicknameForm(forms.Form):
    ...
    def clean_nickname_new(self):
        nickname_new = self.cleaned_data.get('nickname_new', '').strip()    # 获取昵称并 strip()去除空白
        if nickname_new == '':
...
class ChangePasswordForm(forms.Form):
    old_password = forms.CharField(label='旧的的密码',
    ...
    def __init__(self, *args, **kwargs):
        if 'user' in kwargs:
            self.user = kwargs.pop('user')
        super(ChangePasswordForm, self).__init__(*args, **kwargs)   # 将方法写入ChangePasswordForm这个类
...
class ForgotPasswordForm(forms.Form):
    ...
    new_password = forms.CharField(
        label='新的密码',widget=forms.PasswordInput(attrs={'class': 'form-control', 'placeholder': '请输入新的密码'})
    )
